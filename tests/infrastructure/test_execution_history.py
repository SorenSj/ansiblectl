...
def test_history_lists_only_executions_newest_first(tmp_path: Path) -> None:
    ...
    assert records[0].verbosity == 3
    assert records[0].diff is True
    assert records[0].operation == "playbook.syntax_check"
    ...
    assert records[1].verbosity == 0
    assert records[1].diff is False
    assert records[1].operation == "run"
